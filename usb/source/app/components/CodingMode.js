...
export default function CodingMode({ CodingScene, checkBoxHit, getDirectionLabel }) {
  ...
  const getActiveGoalWorldPosition = (items) => {
    const gridX = items?.goal?.x ?? goalPosition[0];
    const gridZ = items?.goal?.z ?? goalPosition[2];
    return [gridX + 0.5, goalPosition[1], gridZ + 0.5];
  };
  ...
  const missionStageConfigs = useMemo(
    () => ({
      ...
      8: {
        allowFreeCommandInsert: true,
        // 7단계 맵 복사 + 6단계처럼 이륙/착륙 사이 자유 명령 추가
        obstacles: [
          { x: 0, z: -2, level: 0, color: "#f59e0b", edgeColor: darkenHex("#f59e0b") },
          { x: 4, z: -1, level: 0, color: "#a78bfa", edgeColor: darkenHex("#a78bfa") },
          { x: 1, z: -4, level: 0, color: "#60a5fa", edgeColor: darkenHex("#60a5fa") },
      ...
      9: {
        // 기존 8단계: 시작 지점 주변을 3칸 높이(level 2)로 둘러, 상승 3회 후에만 이동 가능
        goal: { x: 2, z: -2 },
        obstacles: [
  ...
  // 실행을 끊는 모든 경로(정지/충돌/재실행/언마운트)가 호출하는 단일 취소 지점
  function cancelActiveRun() {
    runTokenRef.current += 1; // 토큰 무효화 → in-flight tick/await가 모두 false로 귀결
    if (timerRef.current) {
    ...
    // 타이머를 지워도 await가 매달리지 않도록, 대기 중 Promise를 즉시 false로 해소
    if (waitResolverRef.current) {
      const settle = waitResolverRef.current;
  ...
  function waitWithToken(ms, token) {
    ...
    return new Promise((resolve) => {
      ...
      // cancelActiveRun()이 대기 중에도 즉시 깨워 await가 매달리지 않게 함
      waitResolverRef.current = settle;
      timerRef.current = setTimeout(() => settle(token === runTokenRef.current), scaledMs);
    });
  }
  ...
  function animateCommandStep(fromPos, toPos, fromRot, toRot, durationMs, token, shouldAbortAt) {
    return new Promise((resolve) => {
      ...
      const tick = (now) => {
        ...
        // 탭 백그라운드로 rAF가 멈췄다 재개되면 큰 간격 발생 → start를 밀어 진행률 점프(워프) 방지
        const delta = now - last;
        last = now;
        if (delta > 200) start += delta;
  ...
  function runCommands() {
    ...
    // 이전 실행/충돌의 좀비 타이머·대기 Promise까지 모두 안전 종료 후 새 토큰 발급
    cancelActiveRun();
    ...
    async function runNext() {
      while (idx < sequence.length) {
        ...
        const activeGoal = getActiveGoalWorldPosition(placedItems);
        const landedOnGoal =
          command.type === "착륙" &&
          !nextIsFlying &&
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      stopRotorLoop();
      // 누수 방지: 오실레이터 정지만으로는 AudioContext가 해제되지 않음 → 명시적 close
      if (store?.ctx) {
        store.ctx.close().catch(() => {});
        store.ctx = null;
        store.master = null;
      }
    };
  ...
  useEffect(() => {
    ...
    if (codingMainMode === "edit") {
      resetDroneOnly();
      setPlacedItems({ obstacles: [], goal: { x: 4, z: -4 } });
      setCommands([{ id: 1, type: "이륙", amount: null }]);
